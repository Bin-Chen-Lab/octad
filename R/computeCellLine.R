...
####### computeCellLine #######
computeCellLine = function(case_id = case_id, expSet = NULL, LINCS_overlaps = TRUE, source = "octad.small", file = NULL, output = FALSE,
                           outputFolder = NULL) {
  # STOPS
  if (missing(case_id)) {
  ...
  if (output == TRUE) {
    return(topline)
    write.csv(correlation.dataframe, file = file.path(cell.line.folder, "CellLineCorrelations.csv"))
  } else {
