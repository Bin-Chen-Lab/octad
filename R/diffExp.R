...
diffExp <- function(case_id = NULL, control_id = NULL, source = "octad.small", file = "octad.counts.and.tpm.h5", normalize_samples = TRUE,
  ...
  counts_phenotype <- rbind(data.frame(sample = case_id, sample_type = "case"), data.frame(sample = control_id, sample_type = "control"))
  ...
  if (DE_method == "DESeq2") {
    ...
    dataCon1 <- count_norm[,c(which(conditions=="case"))]
    dataCon2 <- count_norm[,c(which(conditions== "control"))]
