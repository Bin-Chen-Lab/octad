#' @export
#' @import httr
geneEnrich = function(gene_list = NULL, database_list = NULL, output = FALSE, outputFolder = NULL) {
  ...
  for (j in database_list) {
    ...
    raw_return = httr::GET(paste0("http://amp.pharm.mssm.edu/Enrichr/enrich?userListId=", user_list_id, "&backgroundType=",
                                  j))
    ...
    # some hardcoding stuff to collapse genes into single field or row 30-31 won't work and no parsing for obtained
    # list.  Gene names strickly placed in the [[x]][[y]][6] leaf of list, where [[x]]- name of the database and [[y]]
    # GO entry (eg pathway).
    ...
    # transform list to data.frame, rename columns and compute FDR since enlisted protocol does not gives it. WILL NOT
    # WORK WO line 26-29 transformation!
    result_temp = data.frame(matrix(unlist(raw_return[[j]]), nrow = length(raw_return[[j]]), byrow = TRUE))
    ...
    if (output == TRUE) {
      ...
      write.csv(result_temp, file = file.path(outputFolder, paste0(j, "_GO_enrichment.csv")))
    }
