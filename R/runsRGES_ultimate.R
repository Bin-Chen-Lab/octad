...
#### runsRGES #######
runsRGES <- function(dz_signature = NULL, choose_fda_drugs = FALSE, max_gene_size = 500, cells = NULL, output = FALSE, outputFolder = NULL,
  ...
  dz_cmap_scores = apply(cmap_exp_sig[as.vector(sig.ids)], 2, FUN = function(x) cmap_score_ultimate(dz_genes_up$Symbol, dz_genes_down$Symbol,
                                                                                                    drug_signature = x))
  ...
  random_cmap_scores = apply(cmap_exp_signature, 2, FUN = function(x) cmap_score_ultimate(sample(1:length(dz_genes_up$Symbol),
                                                                                                 replace = TRUE), sample(1:length(dz_genes_down$Symbol), replace = TRUE), drug_signature = x))
  ...
  pred_merged <- pred %>%
    group_by(pert_iname) %>%
    dplyr::summarise(mean = mean(RGES), n = length(RGES), median = median(RGES), sd = sd(RGES))
  ...
  msg=paste("Finished computations in", round(Sys.time() - start_time, 2), units(Sys.time() - start_time), ",writing output")
  message(msg)
  return(pred_merged)
}