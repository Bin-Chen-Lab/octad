...
CTRPv2.auc     = data.table::dcast.data.table(suppressMessages(.eh[["EH7264"]]) ,
...
yaxis = "log(ic50)"
options(warn=-1)
p = ggplot2::ggplot(data = testdf,aes(x = sRGES, y = log10(medIC50))) +
...
htmlwidgets::saveWidget(plotly::as_widget(ic50graph),file.path(normalizePath(dirname(file_name)),basename(file_name)),selfcontained = FALSE)
...
aucgraph = p1 %>% plotly::add_annotations( text=Legend.title,xref="paper",yref="paper",x=1.02,xanchor="left",y=0.8,yanchor="bottom",legendtitle=TRUE,showarrow=FALSE ) %>%
plotly::layout( legend=list(y=0.8, yanchor="top" ) )
