...
topLineEval = function(topline = NULL, mysRGES = NULL, outputFolder = NULL) {
  ...
  CTRPv2.ic50 = data.table::dcast.data.table(suppressMessages(.eh[["EH7264"]]), drugid ~ cellid, value.var = "ic50_recomputed",
  ...
  CTRPv2.auc = data.table::dcast.data.table(suppressMessages(.eh[["EH7264"]]), drugid ~ cellid, value.var = "auc_recomputed",
  ...
  yaxis = "log(ic50)"
  options(warn = -1)
  p = ggplot2::ggplot(data = testdf, aes(x = sRGES, y = log10(medIC50))) + ggplot2::geom_point(aes(color = StronglyPredicted,
                                                                                                   text = paste("Drug: ", pert_iname, "<br>sRGES: ", sRGES))) + ggplot2::geom_smooth(aes(label = ic50rho), method = "lm",
  ...
  ic50graph = p1 %>%
    plotly::add_annotations(text = Legend.title, xref = "paper", yref = "paper", x = 1.02, xanchor = "left", y = 0.8, yanchor = "bottom",
  ...
  htmlwidgets::saveWidget(plotly::as_widget(ic50graph), file.path(normalizePath(dirname(file_name)), basename(file_name)),
                          selfcontained = FALSE)
  # note: files are simply too large to set selfcontained = T. This just causes issues on linux machines.  AUC
  ...
  p = ggplot(testdf2, aes(x = sRGES, y = medauc)) + ggplot2::geom_point(aes(color = StronglyPredicted, text = paste("Drug: ",
                                                                                                                    pert_iname, "<br>sRGES: ", sRGES))) + ggplot2::geom_smooth(aes(label = aucrho), method = "lm", se = FALSE, color = "black",
                                                                                                                                                                               size = 0.5) + ggplot2::scale_color_discrete(name = Legend.title) + ggplot2::labs(x = xaxis, y = yaxis, title = Title) +
  ...
  htmlwidgets::saveWidget(plotly::as_widget(aucgraph), file.path(normalizePath(dirname(file_name)), basename(file_name)), selfcontained = FALSE)
