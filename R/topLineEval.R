...
topLineEval <- function(topline = NULL, mysRGES = NULL, outputFolder = NULL) {
  ...
  yaxis <- "log(ic50)"
  options(warn = -1)
  p <- ggplot2::ggplot(data = testdf, aes(x = testdf$sRGES, y = log10(testdf$medIC50))) +
    ggplot2::geom_point(aes(
      color = StronglyPredicted,
      text = paste("Drug: ", testdf$pert_iname, "<br>sRGES: ", testdf$sRGES)
    )) +
    ggplot2::geom_smooth(aes(label = ic50rho),
    ...
    ggplot2::theme(
      legend.position = "right", legend.background = element_rect(fill = "#F5F5F5"),
  ...
  ic50graph <- p1 %>%
    plotly::add_annotations(
      text = Legend.title, xref = "paper", yref = "paper", x = 1.02, xanchor = "left",
  ...
  p <- ggplot(testdf2, aes(x = testdf2$sRGES, y = testdf2$medauc)) +
    ggplot2::geom_point(aes(
      color = StronglyPredicted,
