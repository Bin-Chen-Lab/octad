...
#dataFolder : folder with commonly used input data
...
dz_signature <- dz_signature %>% filter(GeneID %in% gene.list)
#write.csv(dz_signature,paste0(outputFolder,'dz_sigUsed.csv'),row.names = F)
#abs(dz_signature$value) > dz_fc_threshold & dz_signature$padj < dz_p_threshold, ]
#dz_signature$up_down = ifelse(dz_signature$logFC > 1, "up", "down")
#need to arrange by log fold size if we're using the top genes
...
if (nrow(dz_genes_up) > max_gene_size){
  #dz_genes_up <- data.frame(GeneID= dz_genes_up[1:max_gene_size,]) %>% left_join(dz_signature,by = 'GeneID')
...
for (exp_id in sig.ids) {
 ...
 # print(count)
  ...
  dz_cmap_scores <- c(dz_cmap_scores, 
                      cmap_score_new(dz_genes_up$GeneID,dz_genes_down$GeneID,
                                     cmap_exp_signature))
}
