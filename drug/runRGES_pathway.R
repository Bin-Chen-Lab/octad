...
for (pathway_id in 1:nrow(pathway_gene_set)){  #nrow(pathway_gene_set)
pathways = unlist(strsplit(pathway_gene_set[pathway_id,1], "\t\t"))
pathway_name = pathways[1]
pathway_genes = as.character(sapply(unlist(strsplit(pathways[2], "\t")), function(x) {unlist(strsplit(x, ","))[1]}))
pathway_genes_valid = pathway_genes[pathway_genes %in% gene.list]
print(paste(pathway_id, pathway_name, length(pathway_genes_valid)))

if (length(pathway_genes_valid) <= 4){ next}
...
write(paste(pathway_set_name, pathway_id, pathway_name, length(pathway_genes_valid) , sep = "\t"), paste0("pathway/", pathway_set_name, "/summary.txt"), append=T)
...
dz_signature = data.frame(GeneID = pathway_genes_valid, up_down = "up")
