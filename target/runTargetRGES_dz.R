...
pred$cor <- 1
pred$RGES <- pred$cmap_score #sapply(1:nrow(pred), function(id){getsRGES(pred$cmap_score[id], pred$cor[id], pred$pert_dose[id], pred$pert_time[id], diff, max(pred$cor))})
pred$pert_iname = pred$pert_desc
...
pred_merged <- ddply(pred,  .(pert_iname),  summarise,
                     ...
                     median = median(RGES),
                     sd = sd(RGES))
pred_merged$sRGES <- pred_merged$median
