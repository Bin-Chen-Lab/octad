...
```{r eval=TRUE} 
#select data
suppressMessages(library(octad))
...
```{r eval=TRUE} 
tsne=.eh[["EH7276"]] #Download file with tsneresults for all samples in the octad.db once. After this it will be cached and no additional download required.
tsne$type <- "others"
...
```{r eval=TRUE} 
res=octad.db::res_example #load differential expression example for HCC vs adjacent liver tissue computed in diffExp() function from previous step
res=subset(res,abs(log2FoldChange)>1&padj<0.001) #load example expression dataset
sRGES=runsRGES(res,max_gene_size=100,permutations=1000,output=FALSE,outputFolder = tempdir())
...
topLineEval(topline = 'HEPG2',mysRGES = sRGES,outputFolder=tempdir())
```
...
```{r eval=TRUE} 
octadDrugEnrichment(sRGES = sRGES, target_type='chembl_targets',enrichFolder=paste0(tempdir(),'/'))
```
